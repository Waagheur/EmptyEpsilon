...
ShipSelectionScreen::ShipSelectionScreen()
{
    ...
    player_ship_list = new GuiListbox(left_container, "PLAYER_SHIP_LIST", [this](int index, string value) {
        ...
        if (ship)
        {
            ...
            {
                LOG(INFO) << "Vaisseau choisi " << ship->getCallSign() << ", avec un code de securite.";
    ...
    password_entry_ok = new GuiButton(password_entry_box, "PASSWORD_ENTRY_OK", tr("Ok"), [this]()
    {
        ...
        if (spectator_button->getValue() == true) {
            ...
            if (password != control_code)
            {
                ...
                LOG(INFO) << "Mot de passe de securite incorrect. Tentative : " << password;
                my_player_info->commandSetShipId(-1);
                // Notify the player.
                ...
                // Password matches.
                LOG(INFO) << "Mot de passe de securite correct.";
                // Set the player ship.
                my_player_info->commandSetShipId(ship->getMultiplayerId());
                // Notify the player.
...
void ShipSelectionScreen::update(float delta)
{
    ...
    stations_label->setText("Select your station (" + std::to_string(count_positions) + ")");
