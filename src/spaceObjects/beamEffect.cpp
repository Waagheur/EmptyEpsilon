...
void BeamEffect::setSource(P<SpaceObject> source, glm::vec3 offset)
{
    if (source)
    {
        ...
        sourceOffset = offset;
        sourceOffset.z = source->getTranslateZ(); //tdelc
        update(0);
    }
    else
    {
...
void BeamEffect::setTarget(P<SpaceObject> target, glm::vec2 hitLocation)
{
    if (target)
    {
        ...
        targetOffset = glm::vec3(hitLocation.x + random(-r/2.0f, r/2.0f), hitLocation.y + random(-r/2.0f, r/2.0f), target->getTranslateZ() + random(-r/4.0f, r/4.0f));
