...
void GuiViewport3D::onDraw(sf::RenderTarget& window)
{
    ...
    projection_matrix = glm::perspective(glm::radians(camera_fov), rect.width / rect.height, 1.f, 999999.f);
    ...
    for(int n=render_lists.size() - 1; n >= 0; n--)
    {
        ...
        auto projection = glm::perspective(glm::radians(camera_fov), rect.width / rect.height, 1.f, 999999.f * (n + 1));
