...
{
    (new GuiShieldsEnableButton(this, "SHIELDS_ENABLE", target_spaceship))->setPosition(0, 0, sp::Alignment::TopLeft)->setSize(GuiElement::GuiSizeMax, 50);
...
void GuiShieldFrequencySelect::onDraw(sp::RenderTarget& renderer)
{
    if (target_spaceship)
    {
        calibrate_button->setEnable(target_spaceship->shield_calibration_delay <= 0.0f);
        new_frequency->setEnable(target_spaceship->shield_calibration_delay <= 0.0f);
    }
...
{
    if ((key.category == "ENGINEERING"  || key.category == "WEAPONS") && target_spaceship)
    {
